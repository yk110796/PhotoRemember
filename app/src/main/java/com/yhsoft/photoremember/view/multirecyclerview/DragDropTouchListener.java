...
import static com.yhsoft.photoremember.R.drawable.drag_frame;
 ...
 * Instance of this class should be added to RecylcerView using {@link RecyclerView#addOnItemTouchListener(OnItemTouchListener)} method.
 * <p/>
 * <p/>
...
public abstract class DragDropTouchListener implements OnItemTouchListener {
    ...
    public DragDropTouchListener(RecyclerView recyclerView, AppCompatActivity activity) {
        this.recyclerView = recyclerView;
        this.activity = activity;
        this.displayMetrics = recyclerView.getResources().getDisplayMetrics();
        this.scrollAmount = (int) (50 / displayMetrics.density);
        this.dragHighlight = recyclerView.getResources().getDrawable(drag_frame);

    }

    public DragDropTouchListener(RecyclerView recyclerView, AppCompatActivity activity, Drawable dragHighlight) {
    ...
    private void switchViewsIfNeeded() {
        ...
        if (aboveView != null && aboveView.getTop() > -1 && mobileViewY < aboveView.getTop()) {
            d(LOG_TAG, format("Got aboveView with top = %s, for position = %s, %s", aboveView.getTop(), abovePos, aboveView));
            doSwitch(aboveView, pos, abovePos);
        }
        if (belowView != null && belowView.getTop() > -1 && mobileViewY > belowView.getTop()) {
            d(LOG_TAG, format("Got belowView with top = %s, for position = %s, %s", belowView.getTop(), belowPos, belowView));
    ...
    private ImageView copyViewAsImage(View v) {
        ...
        Bitmap bitmap = createBitmap(v.getWidth(), v.getHeight(), ARGB_8888);
    ...
    private int[] getViewRawCoords(View locateView) {
        View globalView = activity.findViewById(content);
