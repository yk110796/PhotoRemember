...
import com.yhsoft.photoremember.ui.PlaceItem;
...
public class GeoUtil {
    ...
    public class PlaceDownloadTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... param) {
            PlaceItem mItem;
            String queryString = format(SEARCH_URL + "location=%f,%f&types=%s&radius=%d&sensor=true&key=%s",
            ...
            try {
                ...
                e("Result : " + ret + " Place Array length : " + placeArray.length());
                ...
                for (int i = 0; i < placeArray.length(); i++) {
                    ...
                    e("name: " + mItem.name);
                    e("vicinity: " + mItem.vicinity);
                    e("Lat/Lng: " + mItem.lat + ", " + mItem.lng);
                    ...
                    if (photos != null) {
                        ...
                        if (photoRef != null) {
                            String photo_url = format(PHOTO_URL + "maxwidth=270&photoreference=%s&sensor=true&key=%s",
                                    photoRef, GOOGLE_API_KEY);
                            mItem.photoURL = new URL(photo_url);
                        }
                    }
                    e("Photo URL : " + mItem.photoURL);
                    String detailRef = placeArray.optJSONObject(i).optString("reference");
                    String detail_url = format(DETAIL_URL + "reference=%s&sensor=true&key=%s",
                            detailRef, GOOGLE_API_KEY);
                    mItem.detailURL = new URL(detail_url);
                    e("Detail URL : " + mItem.detailURL);
                    ...
                    try {
                        if (detail_results != null) {
                            ...
                            e("Phone number : " + mItem.phoneNumber);
                        } else {
                            e("detail_result is null");
                        }
        ...
        protected String doInBackground(Void... voids) {
            if (mLatLngList != null && mLatLngList.size() > 0) {
                for (int index = 0; index < mLatLngList.size(); index++) {
                    String queryString = format(GEOCODE_URL + "latlng=%f,%f&sensor=true",
                    ...
                    try {
                        ...
                        e("Result : " + ret + " Address Array length : " + addressArray.length());
